...
function p3_tileClicked(i, j) {
  // Start a ripple effect from the clicked tile and affect surrounding tiles
  const rippleRange = 3;  // Determines how far the ripple will spread
  for (let di = -rippleRange; di <= rippleRange; di++) {
    for (let dj = -rippleRange; dj <= rippleRange; dj++) {
      const dist = Math.sqrt(di * di + dj * dj);
      if (dist <= rippleRange) {
        const key = [i + di, j + dj];
        // Initialize ripple effect with a delay based on distance
        ripples[key] = { active: true, reached: false, delay: dist * 10, originalBrightness: map(noise((i + di) * 0.1, (j + dj) * 0.1), 0, 1, 60, 90) };
      }
    }
  }
}
...
function p3_drawTile(i, j) {
  ...
  // Default color settings based on noise
  let noiseValue = noise(i * 0.1, j * 0.1);
  let baseHue = 204;
  let saturation = 100;
  let brightness = map(noiseValue, 0, 1, 60, 90);

  colorMode(HSB, 360, 100, 100);

  // Check if there is an active ripple at this tile
  if (ripples[key] && ripples[key].active) {
    if (ripples[key].delay > 0) {
      ripples[key].delay -= 1;  // Countdown delay before ripple effect
    } else if (!ripples[key].reached) {
      // Change brightness briefly to simulate the ripple passing through
      brightness = 100;  // Set to a higher brightness to simulate ripple
      ripples[key].reached = true;  // Mark the ripple as having reached this tile
    } else {
      // Reset to original brightness after ripple has passed
      brightness = ripples[key].originalBrightness;
      ripples[key].active = false;  // Deactivate ripple after it has passed
    }
  }
